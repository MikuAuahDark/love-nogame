...
local SUPER_TOAST = love._version_major >= 1 or (love._version_major == 0 and love._version_minor == 10)
...
local function getDPI()
	if love._version < "11.0" and love._version >= "0.9.1" then
...
local function isSubtractiveSupported()
	return SUPER_TOAST or love.graphics.isSupported("subtractive")
end

local function lockSpriteBatch(sb, lock)
	...
	-- do nothing for LOVE 0.10.0 and later
end

local ADDITIVE_BLEND = SUPER_TOAST and "add" or "additive"
local SUBTRACTIVE_BLEND = SUPER_TOAST and "subtract" or "subtractive"
...
local function loadimage(file, name)
	return love.graphics.newImage(base64FileData(file, name))
end
...
function love.load()
	...
	if isSubtractiveSupported() then
		love.graphics.setBackgroundColor(137*MUL, 194*MUL, 218*MUL)
	else
		love.graphics.setBackgroundColor(11*MUL, 88*MUL, 123*MUL)
	end
...
local function draw_grid()
	local blendmode = SUBTRACTIVE_BLEND
	if not isSubtractiveSupported() then
		-- We also change the background color in this case, so it looks OK.
		blendmode = ADDITIVE_BLEND
	end
	...
	love.graphics.setColor(255*MUL, 255*MUL, 255*MUL, 128*MUL)
...
local function draw_background(x, y)
	...
	love.graphics.setColor(255*MUL, 255*MUL, 255*MUL, (64 + 16*intensity)*MUL)
	love.graphics.draw(background.image, bx, by, 0, 0.7, 0.7, 256, 256)
	love.graphics.setColor(255*MUL, 255*MUL, 255*MUL, (32 + 16*intensity)*MUL)
	love.graphics.draw(background.image, bx, by, 0, 0.65, 0.65, 256, 256)
	love.graphics.setBlendMode(ADDITIVE_BLEND)
	love.graphics.setColor(255*MUL, 255*MUL, 255*MUL, (16 + 16*intensity)*MUL)
